...
def build_location_table():
    ...
    for city in cities_apple_df.region.unique():
        ...
        # filter out cities that are present in both the US and outside the US assuming Apple is refering to the larger global city
        country = ""
        state = ""
        county = ""

        if not city_df.empty and not world_city_df.empty:
            #print("{} is present in both global and city level dataset".format(city))
            ...
            if world_city_df.iloc[0]["country"] == "United States":
                ...
                #print(city_df[['city', 'state_name', 'population']])
            ...
            #else:
                #print("{} is NOT in the united states".format(city))
...
if "King County" in us_google_df.sub_region_2.unique():
    print(True)
